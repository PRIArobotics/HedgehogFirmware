#include "acp_handler.h"
#include "acp_opcodes.h"
#include "acp_errorcodes.h"
#include "acp_handler_impl.h"
#include "ringbuffer.h"
#include "digitalInput.h"


static uint8_t subscriptions = 0;


void acp_handler_digital_valueReq(acp_conn_t conn, uint8_t opcode, size_t payloadLength)
{
	uint8_t buf[3];
	uint8_t len = 0;

	uint8_t port = ringbuffer_pop(conn.rxBuffer);
	if(port >= DIGITAL_COUNT)
	{
		acp_error(conn, ERR_DIG_OOR, opcode);
		return;
	}
	uint8_t value = digitalInput_getState(port);
	buf[len++] = DIGITAL_SENSOR_REPLY;
	buf[len++] = port;
	buf[len++] = value;

	ringbuffer_push_multiple(conn.txBuffer, buf, len);
}


void acp_handler_digital_valueSub(acp_conn_t conn, uint8_t opcode, size_t payloadLength)
{
	if(payloadLength > 32)
	{
		acp_error(conn, ERR_INVALID_PAYLOAD, opcode);
		return;
	}

	subscriptions = ringbuffer_pop(conn.rxBuffer);
	ringbuffer_consume(conn.rxBuffer, payloadLength - 1);
}


void acp_digital_sendSubUpdate(acp_conn_t conn)
{
	uint8_t buf[2 + 2 * DIGITAL_COUNT];
	uint8_t len = 0, i;

	buf[len++] = DIGITAL_SENSOR_UPDATE;
	len++;
	for(i = 0; i < DIGITAL_COUNT; i++)
	{
		if((subscriptions >> i) & 0x01)
		{
			buf[len++] = i;
			buf[len++] = (uint8_t)digitalInput_getState(i);
		}
	}
	buf[1] = len - 2; //minus opcode and VPL byte
	//only send if there were subscriptions
	if(buf[1] == 0) return;

	ringbuffer_push_multiple(conn.txBuffer, buf, len);
}


void acp_handler_digital_pullAct(acp_conn_t conn, uint8_t opcode, size_t payloadLength)
{
	if(payloadLength > 32)
	{
		acp_error(conn, ERR_INVALID_PAYLOAD, opcode);
		return;
	}

	uint8_t pullups = ringbuffer_pop(conn.rxBuffer);
	ringbuffer_consume(conn.rxBuffer, payloadLength - 1);

	uint8_t i;
	for(i = 0; i < 8; i++)
	{
		digitalInput_setPullup(i, (pullups >> i) & 0x01);
	}
}
